...
function [gratingid, gratingrect] = CreateProceduralGratingMod(windowPtr, type, width, height, amplitude, backgroundColorOffset)
% [gratingid, gratingrect] = CreateProceduralGratingMod(windowPtr, width, height [, backgroundColorOffset =(0,0,0,0)])
%
% changed function CreateProceduralSineGrating.m  from Psychotoolbox3 
%
%
...
if nargin < 4 || isempty(windowPtr) || isempty(width) || isempty(height)
    error('You must provide "windowPtr", "width" and "height"!');
end

if nargin < 6 || isempty(backgroundColorOffset)
...
s = which('CreateProceduralGratingMod.m');
path = fileparts(s);

% Load standard grating shader:
if strmatch('sin', type)
    gratingShader = LoadGLSLProgramFromFiles([path filesep 'GSGLShaders' filesep 'BasicSineGratingShader'], 1);
else
    gratingShader = LoadGLSLProgramFromFiles([path filesep 'GSGLShaders' filesep 'BasicSquareGratingShader'], 1);
end
...
glUniform4f(glGetUniformLocation(gratingShader, 'baseColor'), amplitude(1), amplitude(2), amplitude(3), amplitude(4));
glUseProgram(0);
