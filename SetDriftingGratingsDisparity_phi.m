...
function [ DG, Param ] = SetDriftingGratingsDisparity_phi( win, DG, Param )
[scriptPath, scriptName] = fileparts(mfilename('fullpath'));

    DG.patch_time    = 1/DG.cyclespersecond/4; % (0.125s for TF=2Hz)
...
% 	DG.phaseincrement = (DG.cyclespersecond * 360) * Param.ifi;   % computes increment of degrees per ifi
    ...
    if DG.sinwave
		DG.gratingtex = CreateProceduralGratingMod(win(1), 'sin', DG.gratingsize(1), DG.gratingsize(2), [0.5 0.5 0.5 0], [0.5 0.5 0.5 0]);
		...
		DG.gratingtex = CreateProceduralGratingMod(win(1), 'square', DG.gratingsize(1), DG.gratingsize(2), [0.5 0.5 0.5 0.0], [0.5 0.5 0.5 0]);
    end
    
%     DG.seqdirections = repmat(1:DG.directions, DG.nPhases,1);
    
%     DG.seqphases = repmat([1:DG.nPhases]', 1,DG.directions);
    seqphases = repmat([1:DG.nPhases]', 1,DG.directions);
    seqphases = repmat(seqphases,2,1);
    DG.seqphases = seqphases;
	for r = 1 : DG.n_reps
		switch Param.seqmode
			case 'random'
				DG.stimseq(r,:) = randperm(DG.directions*DG.nPhases*2);
			case 'sequential'
				DG.stimseq(r,:) = 1 : DG.directions*DG.nPhases*2; 
		end
    end
%     DG.IOPhaseDifferences = round( 0 : 360/DG.nPhases : 360-360/DG.nPhases ) ;
    DG.IOPhaseDifferences = DG.phases ;
    % Phase of left screen at t=0:
%     DG.InitialPhase = round((359-0)*rand(DG.directions*DG.nPhases,DG.n_reps) + 0);
    InitialPhase = zeros(DG.directions*DG.nPhases, DG.n_reps);
    InitialPhase = cat(1, InitialPhase,InitialPhase+45);
    % to have equally distributed random numbers in interval [0 3]:
    randomPhases = round((4.49-0.5)*rand(DG.directions*2*DG.nPhases,DG.n_reps) + 0.5) - 1;
    InitialPhase = InitialPhase + randomPhases*90;
    DG.InitialPhase = InitialPhase;
%     DG.InitialPhase = zeros(DG.directions*DG.nPhases,DG.n_reps);

    if ~isempty(DG.angles) && numel(DG.angles)==DG.directions
        DG.seqangles = repmat(DG.angles+90, DG.nPhases*2,1); % +90 to convert from cartesian to PTB angles
        angles_cartesian = DG.angles;
    else
        DG.seqdirections = repmat(1:DG.directions, DG.nPhases*2,1);
...
%     DG.seqphases = DG.seqphases * DG.PhaseStep;
	Param.stimSeq(end+1,1) = { repmat(DG.stim_id, 1,DG.directions*DG.n_reps*DG.nPhases*2) };
    DG.cnt=1;
