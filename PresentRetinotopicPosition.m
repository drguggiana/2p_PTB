...
function [stimtype, vbl] = PresentRetinotopicPosition( nidaq, win, vbl, Param, stimtype )
...
if Param.Dichoptic && Param.stereoMode==4
    if     strcmp(inputname(5),'RP')
        winStereo = 0;
    elseif strcmp(inputname(5),'RP2')
        winStereo = 1;
    end
	...
	mask=reshape( stimtype.coord_positions(stimtype.seqpositions(stimtype.cnt,1), stimtype.seqpositions(stimtype.cnt,2) ,:) , 1, 4 );
	tempo=[0 0];
%     tic
...
% 	while vbl < vblendtime
		...
		for d=1:stimtype.directions
			...
			for f = 1 : fix(stimtype.stimulus_time_fr/stimtype.directions)
				...
				Screen('DrawTexture', win, stimtype.gratingtex, [], mask, angle, [], [], [], [], Param.rotateMode, [stimtype.phase, stimtype.freq_cppx, stimtype.amplitude, 0]);
                
				if Param.usePhotodiode 
                    if Param.stereoMode==4
