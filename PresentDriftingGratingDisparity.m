...
function [stimtype, vbl] = PresentDriftingGratingDisparity( nidaq, screen, vbl, Param, stimtype )
...
phase1running = phase1;
phase2running = phase2;
...
%     phase1 = phase1 + phaseincrement;
    phase1running = phase1running + phaseincrement;
    phase2running = phase2running + phaseincrement;
...
%         Screen('DrawTexture', win, stimtype.masktex{eye}, [], []);
    ...
    Screen('SelectStereoDrawBuffer', win, 1);
    eye = 1;
	Screen('DrawTexture', win, stimtype.gratingtex, [], stimtype.gratingRect(eye,:), angle2, [], [], [], [], Param.rotateMode, [phase2running+deltaphase, stimtype.freq_cppx(2), stimtype.amplitude, 0]);
    if stimtype.drawMask == 1
        Screen('BlendFunction', win, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		% Draw mask over grating:
		Screen('DrawTexture', win, stimtype.masktex{eye}, [], stimtype.gratingRect(eye,:));
