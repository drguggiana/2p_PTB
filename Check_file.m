%% Check a particular file for proper writing/ stim ID etc INCOMPLETE
...
%         num_scans_im = frame_averaging * im_frames;
        ...
        switch exp_type
            case 0
                [~,eye2_data] = load_eye_monitor_data(fullfile(eye2_file.folder,eye2_file.name),0);
            case 1
...
%                 end
                ...
                for trials = 1:subtrial_num
                    ...
                    if isnan(trial_start)
                        ...
                        trial_mat(:,:,reps,stim) = permute(R_detrend(trial_start:trial_start + trial_duration-1,:),[2 1 3 4]);
                        %get the corresponding cam info
                        trial_cam(:,reps,stim,:)= cam_data(:,trial_start:trial_start + trial_duration-1)';
                    end
